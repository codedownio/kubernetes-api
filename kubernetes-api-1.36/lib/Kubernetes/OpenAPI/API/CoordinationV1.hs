...
-- | @application/cbor@
instance Produces CreateNamespacedLease MimeCbor
-- | @application/yaml@
...
instance HasOptionalParam DeleteCollectionNamespacedLease IgnoreStoreReadErrorWithClusterBreakingPotential where
...
instance HasOptionalParam DeleteCollectionNamespacedLease ShardSelector where
...
-- | @application/cbor@
instance Produces DeleteCollectionNamespacedLease MimeCbor
-- | @application/yaml@
...
-- | /Optional Param/ "ignoreStoreReadErrorWithClusterBreakingPotential" - if set to true, it will trigger an unsafe deletion of the resource in case the normal deletion flow fails with a corrupt object error. A resource is considered corrupt if it can not be retrieved from the underlying storage successfully because of a) its data can not be transformed e.g. decryption failure, or b) it fails to decode into an object. NOTE: unsafe deletion ignores finalizer constraints, skips precondition checks, and removes the object from the storage. WARNING: This may potentially break the cluster if the workload associated with the resource being unsafe-deleted relies on normal deletion flow. Use only if you REALLY know what you are doing. The default value is false, and the user must opt in to enable it
instance HasOptionalParam DeleteNamespacedLease IgnoreStoreReadErrorWithClusterBreakingPotential where
  applyOptionalParam req (IgnoreStoreReadErrorWithClusterBreakingPotential xs) =
    req `addQuery` toQuery ("ignoreStoreReadErrorWithClusterBreakingPotential", Just xs)
...
-- | @application/cbor@
instance Produces DeleteNamespacedLease MimeCbor
-- | @application/yaml@
...
-- | /Optional Param/ "shardSelector" - shardSelector restricts the list of returned objects using a CEL-based shard selector expression. The format uses the shardRange() function combined with || (logical OR) to specify one or more hash ranges:    shardRange(object.metadata.uid, '0x0', '0x8000000000000000')   shardRange(object.metadata.uid, '0x0', '0x8000000000000000') || shardRange(object.metadata.uid, '0x8000000000000000', '0x10000000000000000')  Field paths use CEL-style object-rooted syntax (e.g. \"object.metadata.uid\"), NOT the fieldSelector format (\"metadata.uid\"). Currently supported paths:   - object.metadata.uid   - object.metadata.namespace  hexStart and hexEnd are single-quoted CEL string literals with a '0x' prefix, defining the inclusive lower and exclusive upper bounds over the 64-bit FNV-1a hash space. The full range is [0x0, 0x10000000000000000), where the exclusive upper bound equals 2^64.  Examples:   2-shard split:     shard 0: shardRange(object.metadata.uid, '0x0000000000000000', '0x8000000000000000')     shard 1: shardRange(object.metadata.uid, '0x8000000000000000', '0x10000000000000000')   4-shard split:     shard 0: shardRange(object.metadata.uid, '0x0000000000000000', '0x4000000000000000')     shard 1: shardRange(object.metadata.uid, '0x4000000000000000', '0x8000000000000000')     shard 2: shardRange(object.metadata.uid, '0x8000000000000000', '0xc000000000000000')     shard 3: shardRange(object.metadata.uid, '0xc000000000000000', '0x10000000000000000')  This is an alpha field and requires enabling the ShardedListAndWatch feature gate.
instance HasOptionalParam ListLeaseForAllNamespaces ShardSelector where
  applyOptionalParam req (ShardSelector xs) =
    req `addQuery` toQuery ("shardSelector", Just xs)
...
instance Produces ListLeaseForAllNamespaces MimeCborSeq
-- | @application/json@
...
-- | @application/cbor@
instance Produces ListLeaseForAllNamespaces MimeCbor
-- | @application/yaml@
...
-- | /Optional Param/ "shardSelector" - shardSelector restricts the list of returned objects using a CEL-based shard selector expression. The format uses the shardRange() function combined with || (logical OR) to specify one or more hash ranges:    shardRange(object.metadata.uid, '0x0', '0x8000000000000000')   shardRange(object.metadata.uid, '0x0', '0x8000000000000000') || shardRange(object.metadata.uid, '0x8000000000000000', '0x10000000000000000')  Field paths use CEL-style object-rooted syntax (e.g. \"object.metadata.uid\"), NOT the fieldSelector format (\"metadata.uid\"). Currently supported paths:   - object.metadata.uid   - object.metadata.namespace  hexStart and hexEnd are single-quoted CEL string literals with a '0x' prefix, defining the inclusive lower and exclusive upper bounds over the 64-bit FNV-1a hash space. The full range is [0x0, 0x10000000000000000), where the exclusive upper bound equals 2^64.  Examples:   2-shard split:     shard 0: shardRange(object.metadata.uid, '0x0000000000000000', '0x8000000000000000')     shard 1: shardRange(object.metadata.uid, '0x8000000000000000', '0x10000000000000000')   4-shard split:     shard 0: shardRange(object.metadata.uid, '0x0000000000000000', '0x4000000000000000')     shard 1: shardRange(object.metadata.uid, '0x4000000000000000', '0x8000000000000000')     shard 2: shardRange(object.metadata.uid, '0x8000000000000000', '0xc000000000000000')     shard 3: shardRange(object.metadata.uid, '0xc000000000000000', '0x10000000000000000')  This is an alpha field and requires enabling the ShardedListAndWatch feature gate.
instance HasOptionalParam ListNamespacedLease ShardSelector where
  applyOptionalParam req (ShardSelector xs) =
    req `addQuery` toQuery ("shardSelector", Just xs)
...
instance HasOptionalParam ListNamespacedLease Watch where
  applyOptionalParam req (Watch xs) =
    req `addQuery` toQuery ("watch", Just xs)
-- | @application/cbor-seq@
instance Produces ListNamespacedLease MimeCborSeq
-- | @application/json@
...
-- | @application/cbor@
instance Produces ListNamespacedLease MimeCbor
-- | @application/yaml@
...
instance Consumes PatchNamespacedLease MimeApplyPatchcbor
...
-- | @application/cbor@
instance Produces PatchNamespacedLease MimeCbor
-- | @application/yaml@
...
-- | @application/cbor@
instance Produces ReadNamespacedLease MimeCbor
-- | @application/yaml@
...
-- | @application/cbor@
instance Produces ReplaceNamespacedLease MimeCbor
-- | @application/yaml@
